...
ai = socket.getaddrinfo(deviceRoutingPrefix, int(configStore.getConfig("osc-recieveport")))[0]
    ...
    try:
        for res in poll(1):
            if res[1] & (select.POLLERR | select.POLLHUP):
                ...
                handle_osc(data, caddr, dispatch=oscMessageRecieved)
        loopActions()
...
print("Closing socket and sleeping for 30s")
sock.close()
time.sleep(30)